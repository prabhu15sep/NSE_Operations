...
def test_run():
    ...
    df_Stock = rd.get_data(list_nse50[0:50],'2018-08-13',200)        #Extract Data from a given date to past x no of traded days
    ...
    df_filter = df_filter.rename(columns={'13/08/2018': 'A~CP-3/8'}) 
    ...
    ########################Citeria 1 to Buy###########
    df_filter['A<(B/C) '] = np.where(((df_filter['A~CP-3/8'] <= df_filter['B~50DA']) & \
                                      (df_filter['A~CP-3/8'] >= (df_filter['B~50DA'])*0.95)) \
                                    |((df_filter['A~CP-3/8'] <= df_filter['C~200DA']) & \
                                      (df_filter['A~CP-3/8'] >= (df_filter['C~200DA'])*0.95)), 1 , 0)
    ...
    ########################Citeria to Sell###########
    df_filter['A>(B/C) '] = np.where(((df_filter['A~CP-3/8'] >= df_filter['B~50DA']) & \
                                      (df_filter['A~CP-3/8'] <= (df_filter['B~50DA'])*1.05)) \
                                    |((df_filter['A~CP-3/8'] >= df_filter['C~200DA']) & \
                                      (df_filter['A~CP-3/8'] <= (df_filter['C~200DA'])*1.05)), 1 , 0)
